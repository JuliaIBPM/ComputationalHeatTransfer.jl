...
zero_body_state(sys::HeatConduction) = zero_body_state(sys.bodies)


## Right-hand sides for rigid-body motion equations ##

function rigid_body_rhs!(dx::Vector{T},x::Vector{T},sys::HeatConduction,t::Real) where {T<:Real}
...
rigid_body_rhs(x::Vector{T},sys::HeatConduction,t::Real) where {T<:Real} =
